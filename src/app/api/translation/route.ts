...
export var POST = auth(async function (request: NextAuthRequest) {
	...
	try {
		let { text } = await generateText({
			...
			system: `You are a language specialist. In a precise and professional manner, translate any sentence you receive into ${nativeLanguage}, and try to translate it to align more with its literal meaning, with less or only mild paraphrasing. Be careful: Chinese characters can appear in Japanese, but it is still Japanese, not Chinese. Otherwise, if the sentence is already in ${nativeLanguage}, do nothing and simply return it as is. Do not return anything other than the translation text.`,
